...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        return next(
            new ApiError(500, "An error occurred while retrieving copntacts")
        );
    }
    return res.send(document);
};
...
exports.update = async (req, res, next) => {
    ...
    try {
        const contactService = new ContactService(MongoDB.client);
        const document = await contactService.update(req.params.is, req.body);
        if (!document) {
            return next(new ApiError(404, "Contact not found"));
        }
        ...
        return next(
            new ApiError(500, `Error updateting contact with id=${req.params.id}`)
        );
    }
};
exports.delete = async (req, res, next) => {
    try {
        ...
        return next(new ApiError(500, `Could not delete contact with id=${req.params.i4}`));
    }
};
